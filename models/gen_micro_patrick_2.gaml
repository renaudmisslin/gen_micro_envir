...
global {
	...
	reflex dynamic when: cycle = 1 {
		...
		write "Initialisation des cellules environnement ";
	//	do init_environment_cells;
		
		/******************************************************************************************************************************************************************/
		// Initialisation des routes et des zones autour (chaussees et zones proches)
		write "Creation des routes ";
		do create_roads;
		
		/******************************************************************************************************************************************************************/
		// Calcul de l'espace non bati (végétation, eau, routes)
		write " Calcul de l'espace non bati (végétation, eau, routes)";
		do compute_non_builtable_space;
		
		// Calcul de l'espace libre à batir dans chaque cell_env et création des ménages
		write "creation des menages";
		do create_menage;
		
		// Création de bâtiments
		write "creation des batiments";
		do create_buildings;
		
		// Logement des ménages
		write "localisation des menages";
		do locate_menages;
		
		//Calcul du nombre de GLP par cell_env
		write "calcul des gites";
		do compute_gites;
	...
	//	do save_data;
		...
		do compare_with_obs_data;
		
		write "Fin du modèle";
	}
	

	action init_environment_cells {
	...
	action create_roads {
		create route from: shp_routes with: [type::string(read("fclass")), pont::string(read("bridge")), tunnel::string(read("tunnel"))] {
			if not(self overlaps bangkhutien) {do die;}
		}
		ask route {
			do init_routes;
		}
	...
	action compute_non_builtable_space {
	...
	action compare_with_obs_data {
		...
		loop cell_valid over: grid_valid  {
			...
			list<geometry> bati_est_cell <- batiment overlapping cell_valid;
			list<geometry> bati_obs_cell <- batiment_obs overlapping cell_valid;
			...
			if (n_est > 0 and n_obs > 0) {
				...
				mat_results[cpt_results,5] <- n_bat_est_inter_pond / n_est;
				mat_results[cpt_results,6] <- n_bat_est_inter_pond / n_est;
				mat_results[cpt_results,7] <- n_bat_obs_inter_pond / n_obs;
		...
		save mat_results to: "../results/results_valid.csv";
	}
...
experiment genmicro type: gui {
	output {
		display main_display  {
			graphics "grid_valid" {loop g over: grid_valid {draw g color: #yellow border:#black;}}
			//grid cell_env;
